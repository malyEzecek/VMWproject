...
const YEAR_MIN = 1960;
const YEAR_MAX = 2018;
const CONSUMPTION_MIN = 4;
const CONSUMPTION_MAX = 15;

module.exports.generate = function(size) {
    let carDB = [];
    for (let i = 0; i < size; i++) {
      const manufacturer = cars[getRandomInt(0, CARS.length - 1)];
      ...
      // Get rid of UTF-8 characters
      const carName = (manufacturerName + " " + model).normalize('NFD').replace(/[\u0300-\u036f]/g, "");
    
      carDB.push({
        _id: i,
        name: carName,
        price: getRandomInt(PRICE_MIN, PRICE_MAX),
        year: getRandomInt(YEAR_MIN, YEAR_MAX),
        consumption: getRandomInt(CONSUMPTION_MIN, CONSUMPTION_MAX),
        fuel: getRandomInt(0, FUELS.length - 1),
        parkingAssistant: getRandomBoolean(),
        xenons: getRandomBoolean(),
        ABS: getRandomBoolean(),
        airConditioning: getRandomBoolean()
      });
    }
    
    fs.writeFile("./cars_" + size + ".json", JSON.stringify(carDB), function(err) {
        if(err) {
